...
fn runtime(id: &str) -> Runtime {
	...
	} else if id.starts_with("oracle-rococo") {
		Runtime::OracleRococo	
...
fn load_spec(id: &str) -> std::result::Result<Box<dyn ChainSpec>, String> {
	...
	Ok(match id {
		...
		// -- Oracle on Rococo
		"oracle-rococo-dev" =>
			Box::new(chain_spec::oracle::oracle_rococo_development_config()),
		"oracle-rococo-local" =>
			Box::new(chain_spec::oracle::oracle_rococo_local_config()),
		"oracle-rococo-genesis" => Box::new(chain_spec::oracle::oracle_rococo_config()),
		"oracle-rococo" =>
			Box::new(chain_spec::oracle::OracleRococoChainSpec::from_json_bytes(
				&include_bytes!("../../parachains/chain-specs/oracle-rococo.json")[..],
			)?),
		...
		path => {
			...
			match path.runtime() {
				...
				Runtime::OracleRococo =>
					Box::new(chain_spec::oracle::OracleRococoChainSpec::from_json_file(path)?),	
...
impl SubstrateCli for Cli {
	...
	fn native_runtime_version(chain_spec: &Box<dyn ChainSpec>) -> &'static RuntimeVersion {
		match chain_spec.runtime() {
			...
			Runtime::OracleRococo => &oracle_rococo_runtime::VERSION,
			Runtime::BridgeHub(bridge_hub_runtime_type) =>
...
macro_rules! construct_async_run {
	(|$components:ident, $cli:ident, $cmd:ident, $config:ident| $( $code:tt )* ) => {{
		...
		match runner.config().chain_spec.runtime() {
			...
			Runtime::OracleRococo => {
				runner.async_run(|$config| {
					let $components = new_partial::<oracle_rococo_runtime::RuntimeApi, _>(
						&$config,
						crate::service::oracle_rococo_build_import_queue,
					)?;
					let task_manager = $components.task_manager;
					{ $( $code )* }.map(|v| (v, task_manager))
				})
	...
	match &cli.subcommand {
		...
		Some(Subcommand::TryRuntime(cmd)) => {
			...
			match runner.config().chain_spec.runtime() {
				...
				Runtime::OracleRococo => runner.async_run(|_| {
					Ok((
						cmd.run::<Block, HostFunctionsOf<crate::service::OracleRococoRuntimeExecutor>>(),
						task_manager,
					))
				}),
		...
		None => {
			...
			runner.run_node_until_exit(|config| async move {
				...
				match config.chain_spec.runtime() {
					...
					// .map(|r| r.0)
					// .map_err(Into::into),
					// Runtime::OracleRococo => crate::service::start_oracle_rococo_node(
					// 	config,
					// 	polkadot_config,
					// 	collator_options,
					// 	id,
...
mod tests {
	...
	fn test_resolve_runtime_for_different_configuration_files() {
		...
		let path = store_configuration(
			&temp_dir,
			Box::new(crate::chain_spec::oracle::oracle_rococo_local_config()),
		);
		assert_eq!(Runtime::OracleRococo, path.runtime());
	}
}
