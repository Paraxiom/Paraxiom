...
// Native oracle executor instance.
pub struct OracleRococoRuntimeExecutor;

impl sc_executor::NativeExecutionDispatch for OracleRococoRuntimeExecutor {
	type ExtendHostFunctions = frame_benchmarking::benchmarking::HostFunctions;

	fn dispatch(method: &str, data: &[u8]) -> Option<Vec<u8>> {
		oracle_rococo_runtime::api::dispatch(method, data)
	}

	fn native_version() -> sc_executor::NativeVersion {
		oracle_rococo_runtime::native_version()
	}
}
...
#[sc_tracing::logging::prefix_logs_with("Parachain")]
async fn start_oracle_rococo_node_impl<RuntimeApi, RB, BIQ, BIC>(
	parachain_config: Configuration,
	polkadot_config: Configuration,
	collator_options: CollatorOptions,
	para_id: ParaId,
	...
	) -> Result<Box<dyn ParachainConsensus<Block>>, sc_service::Error>,
{
	...
	let rpc_builder = {
		...
		Box::new(move |deny_unsafe, _| {
			...
			crate::rpc::create_oracle_rococo(deps).map_err(Into::into)
		})
	};
...
pub fn oracle_rococo_build_import_queue(
	client: Arc<ParachainClient<oracle_rococo_runtime::RuntimeApi>>,
	block_import: ParachainBlockImport<oracle_rococo_runtime::RuntimeApi>,
	config: &Configuration,
	telemetry: Option<TelemetryHandle>,
	task_manager: &TaskManager,
) -> Result<
	sc_consensus::DefaultImportQueue<Block, ParachainClient<oracle_rococo_runtime::RuntimeApi>>,
	sc_service::Error,
> {
	let slot_duration = cumulus_client_consensus_aura::slot_duration(&*client)?;
...
pub async fn start_oracle_rococo_node(
	parachain_config: Configuration,
	polkadot_config: Configuration,
	collator_options: CollatorOptions,
	para_id: ParaId,
	...
	TaskManager,
	Arc<ParachainClient<oracle_rococo_runtime::RuntimeApi>>,
)> {
	start_oracle_rococo_node_impl::<oracle_rococo_runtime::RuntimeApi, _, _, _>(
		parachain_config,
		polkadot_config,
		collator_options,
		para_id,
		|_| Ok(RpcModule::new(())),
		oracle_rococo_build_import_queue,
		|client,
		 block_import,
		 prometheus_registry,
		 telemetry,
