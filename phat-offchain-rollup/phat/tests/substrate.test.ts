...
describe('Substrate Offchain Rollup', () => {
    ...
    const httpRpc: string =  "http://127.0.0.1:41315";
    ...
    describe('SubPriceFeed', () => {
        ...
        it('should be configurable', async function() {
            ...
            const feedConfig2 = await priceFeed2.tx
                ...
                .signAndSend(alice, {nonce: -1});
            this.timeout(1000*30_000);
            console.log('Feed configured', feedConfig2.toHuman());
            ...
            const feedConfig3 = await priceFeed3.tx
                ...
                .signAndSend(alice, {nonce: -1});
            this.timeout(1000*30_000);
            console.log('Feed configured', feedConfig3.toHuman());
            ...
            const feedConfig4 = await priceFeed4.tx
                ...
                .signAndSend(alice, {nonce: -1});
            this.timeout(1000*30_000);
            console.log('Feed configured', feedConfig4.toHuman());
            ...
            const feedConfig5 = await priceFeed5.tx
                ...
                .signAndSend(alice, {nonce: -1});
            this.timeout(1000*30_000);
            console.log('Feed configured', feedConfig5.toHuman());
            
            const feedConfig6 = await priceFeed6.tx
                .config(txConf, httpRpc, 100, secretBob as any, 'polkadot', 'usd')
                .signAndSend(alice, {nonce: -1});
            console.log('Feed configured', feedConfig6.toHuman());
            this.timeout(1000*30_000);

            
            // // Init the rollup on the blockchain
            const init = await priceFeed.query.maybeInitRollup(certAlice, {});
            // const ini2 = await priceFeed2.query.maybeInitRollup(certAlice, {});
        ...
        it('can submit tx', async function() {
            ...
            // const feed6 = await priceFeed6.query.feedPrice(certAlice, {});
    ...
    describe('Sub0Factory', () => {
        before(async function() {
            ...
            // Deploy contract
            // sub1 = await sub0Factory.instantiate('default', [], {transferToCluster: 1e12});
            // console.log('Sub0Factory deployed at', sub0.address.toString());
        });
        ...
        it('can be configured', async function() {
            ...
            await delay(4*1000);
            // const sub0Config2 = await sub1.tx
            //     .config(txConf, httpRpc, 100, secretBob as any, priceFeedCodeHash)
            //     .signAndSend(alice, {nonce: -1});
            // console.log('Sub0Factory configured', sub0Config.toHuman());
            // await delay(4*1000);
        ...
        it('can deploy price feeds', async function() {
            ...
            let deploy2 = await api.tx.utility.batchAll([
                sub0.tx.deployPriceFeed(txConf, 'feed1', 'polkadot', 'usd'),
                sub0.tx.deployPriceFeed(txConf, 'feed2', 'bitcoin', 'usd'),
            ]).signAndSend(alice, {nonce: -1});
            ...
            priceFeed = await priceFeedFactory.attach(feed1Addr);
        });

        it('can trigger a rollup', async function() {
            // Init the rollup on the blockchain
            const init = await priceFeed.query.maybeInitRollup(certAlice, {});
            // const init2 = await priceFeed1.query.maybeInitRollup(certAlice, {});
            // expect(init.result.isOk).to.be.true;
            // expect(init.output.isOk).to.be.true;
            // expect(init.output.asOk.isSome).to.be.true;
            await delay(3*1000);
            ...
            // Trigger a rollup
            const feed = await priceFeed.query.feedPrice(certAlice, {});
            // const feed2 = await priceFeed1.query.feedPrice(certAlice, {});
            // const feed3 = await priceFeed1.query.feedPrice(certAlice, {});
            // const feed4 = await priceFeed1.query.feedPrice(certAlice, {});
            // const feed5 = await priceFeed1.query.feedPrice(certAlice, {});
            // const feed6 = await priceFeed1.query.feedPrice(certAlice, {});
            // expect(feed.result.isOk).to.be.true;
            // expect(feed.output.isOk).to.be.true;
            // expect(feed.output.asOk.isSome).to.be.true;
            await delay(3*1000);
